...
class AuthNotifier extends Notifier<AuthState> {
  ...
  Future<void> login({
    ...
    state = state.copyWith(isLoading: true, clearSessionExpired: true);

    try {
      final response = await _authService.loginAdmin(
        AdminLoginRequest(username: username.trim(), password: password),
      );
...
class AuthState {
  ...
  AuthState copyWith({
    ...
    return AuthState(
      ...
      isSessionExpired: clearSessionExpired
          ? false
